...
const StadiumContentSearch = () => {
    ...
    const handleSearch = async () => {
        ...
        params.append("sports_category", sports_category);
        params.append(
            "stadium_location",
            stadium_location === "ALL" ? "" : stadium_location.slice(0, 2)
        );
        params.append("stadium_name", stadium_name);
        params.append("reservation", reservation);

        navigate(`/stadiums?${params.toString()}`);

        console.log("Query sent to the server: ", params.toString());
